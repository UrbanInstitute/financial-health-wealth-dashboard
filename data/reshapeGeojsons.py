...
def reshapeGeojson(geoidType, infile, outfile):
    ...
    for d in inData:
        ...
        properties = {} # only want to keep Mapbox-compatible geo ID and geo type (PUMA or city)
        properties["geo_id"] = d["properties"]["id"]

        if geoidType == "puma":
            properties["type"] = "PUMA"
        elif geoidType == "city":
...
if __name__ == "__main__":
    reshapeGeojson("puma", "pumas-id.json", "pumas_bboxes.json")
    reshapeGeojson("city", "cities-id.json", "cities_bboxes.json")